...
/**
 * Moveable box. A movable box has a descendant element with <code>caption</code> CSS class. User can press on it,
 * hold and move mouse to change box position within the page. While moving the box, a CSS class named
 * <code>moving</code> is active. It can be used to hide box content if the box is too heavy, considering user agent
 * should redraw the box following mouse movement; <code>moving</code> CSS class can be also used to mark somehow the
 * box so that user to be aware he can move it.
 * <p>
 * If box has a descendant element with <code>box-close</code> CSS class register {@link #_onBoxClose} which will hide
 * entire box, of course on user click.
 * <p>
 ...
 * @assert box element is fixed positioned and has a child with class <code>caption</code>.
 */
js.widget.Box = function(ownerDoc, node) {
	 ...
	 * @type js.dom.Element
	 */
	this._caption = this.getByCssClass("caption");
	...
	var boxClose = this.getByCssClass("box-close");
	if (boxClose !== null) {
		boxClose.on("click", this._onBoxClose, this);
	}

	/**
	 * Track box top position while moving, relative to this box containing block. Remember that containing block is the
	...
	/**
	 * Track box left position while moving, relative to this box containing block. Remember that containing block is
...
js.widget.Box.prototype = {
	...
	/**
	 * Enter moving state. Compute maximum allowed values for X and Y coordinates, add <code>moving</code> CSS class
	 * and attach mouse move and mouse up events to document.
	 */
	 ...
	 * <code>moving</code> CSS class is erased.
	 * 
	 * @param js.event.Event ev mouse up event.
	 */
	...
	/**
	 * On box close event listener. Triggered when user press on <code>box closing element</code>, that is, a box
	 * descendant having <code>box-close</code> CSS class. This listener just hide entire box.
	 * 
	 * @param js.event.Event ev mouse up event.
	 */
	...
	/**
	 * Set box position to current coordinates. Current coordinates are computed in real time based on mouse movement.
	 * Note that coordinates are relative to containing block.
	 * 
	 * @see #_x
	 * @see #_y
	 */
